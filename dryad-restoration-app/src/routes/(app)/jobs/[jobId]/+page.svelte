<script lang="ts">
  ...
  function handleTaskCompletedFromModal(event: CustomEvent<{taskId: string, data?: any}>) {
    ...
    if (taskId === 'log_final_readings') {
      // This is handled elsewhere by the log entry processor
      console.log('Moisture readings logged');
      
      // Force update finalReadingsLogged to ensure consistency
      if ($currentJob && $currentJob.completionTasks) {
        updateTaskStatus('finalReadingsLogged', true);
      }
      closeTaskModal();
    } else if (taskId === 'upload_after_photos') {
      // Add a proper log entry for the uploaded photos
      if (data && data.url) {
        addLogEntry({
          jobId: $currentJob?.id || '',
          userId: $currentUser?.id || '',
          ...
          console.log('Photo log entry created from task data');
          // Force update afterPhotosTaken to ensure consistency
          if ($currentJob && $currentJob.completionTasks) {
