<script lang="ts">
  ...
  import InsuranceTaskChecklist from '$lib/components/jobs/InsuranceTaskChecklist.svelte';
  import JobInsuranceTaskModal from '$lib/components/jobs/JobInsuranceTaskModal.svelte';
  ...
  // Add state for insurance task modal
  let showInsuranceTaskModal = false;
  let currentInsuranceTaskId = '';
  ...
  // Handle task clicks in the insurance task checklist
  function handleInsuranceTaskClicked(event: CustomEvent<{taskId: string, action: string}>) {
    const { taskId, action } = event.detail;
    currentInsuranceTaskId = taskId;
    showInsuranceTaskModal = true;
  }

  // Handle insurance task completion
  function handleInsuranceTaskComplete(event: CustomEvent<{job: Job}>) {
    const { job } = event.detail;
    currentJob.set(job);
    showInsuranceTaskModal = false;
  }

  // Close the insurance task modal
  function closeInsuranceTaskModal() {
    showInsuranceTaskModal = false;
  }
      ...
      <div class="p-6 bg-gray-50">
        ...
        <div class="mt-6">
          ...
          {:else}
            ...
            <!-- INSURANCE TASK CHECKLIST AREA -->
            {#if $currentUser?.role === Role.ADMIN || $currentUser?.role === Role.OFFICE}
              <div class="mt-8 border-t pt-6">
                <InsuranceTaskChecklist 
                  job={$currentJob} 
                  on:taskClicked={handleInsuranceTaskClicked}
                />
              </div>
            {/if}
            <!-- END INSURANCE TASK CHECKLIST AREA -->
          {/if}
        </div>
        ...
        <!-- Insurance Task Modal -->
        {#if showInsuranceTaskModal && $currentJob}
          <JobInsuranceTaskModal
            job={$currentJob}
            taskId={currentInsuranceTaskId}
            isOpen={showInsuranceTaskModal}
            on:taskComplete={handleInsuranceTaskComplete}
            on:close={closeInsuranceTaskModal}
          />
        {/if}
      </div>
    </div>
