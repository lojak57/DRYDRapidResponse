...
<div>
  ...
  <div class="mt-6 border-t border-gray-200 pt-6">
    <h3 class="text-md font-medium text-gray-700 mb-3">Special Processing</h3>
    <div class="flex items-center">
      <input 
        type="checkbox" 
        id="insuranceMode"
        bind:checked={formData.insuranceMode}
        class="h-4 w-4 text-dryd-blue focus:ring-dryd-blue border-gray-300 rounded"
        {disabled}
      >
      <label for="insuranceMode" class="ml-2 flex items-center">
        <span class="font-medium text-gray-700">Insurance Mode</span>
        <span class="ml-1 text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded">New</span>
      </label>
    </div>
    
    {#if formData.insuranceMode}
      <p class="text-sm text-gray-600 mt-2 ml-6">
        Insurance mode enables specialized workflow tasks for processing insurance claims.
        This will add additional steps for document collection and submission to carriers.
      </p>
    {/if}
  </div>
