<script lang="ts">
  ...
  // Customer and technicians data stores
  ...
  onMount(async () => {
    try {
      ...
      console.error(`Error loading data for job ${job.id}:`, err);
    }
  });
</script>
...
<div class="card-glass rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-200 ease-in-out card-shadow">
  ...
  <div class="p-5 bg-white">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
      ...
      <!-- Scheduled Date - Show for SCHEDULED and IN_PROGRESS jobs -->
      {#if (job.status === JobStatus.SCHEDULED || job.status === JobStatus.IN_PROGRESS) && job.scheduledStartDate}
        <div class="flex items-start">
          <svg class="h-5 w-5 text-gray-500 flex-shrink-0 mt-0.5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div>
            ...
            <p class="text-gray-900 font-medium">{formatDate(job.scheduledStartDate)}</p>
          </div>
        </div>
      {/if}

      <!-- Assigned Technicians - Show for SCHEDULED and IN_PROGRESS jobs -->
      {#if (job.status === JobStatus.SCHEDULED || job.status === JobStatus.IN_PROGRESS) && job.assignedUserIds && job.assignedUserIds.length > 0}
        <div class="flex items-start">
          <svg class="h-5 w-5 text-gray-500 flex-shrink-0 mt-0.5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
          </svg>
          <div>
            <p class="text-sm font-medium text-gray-600">Assigned Technicians</p>
            ...
            {:else if job.assignedUserIds.length > 0}
              <p class="text-gray-900 font-medium">Loading technician details...</p>
            {:else}
              <p class="text-gray-900 font-medium">No technicians assigned</p>
            {/if}
          </div>
        </div>
      {/if}
