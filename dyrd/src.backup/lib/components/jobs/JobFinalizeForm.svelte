<script lang="ts">
  ...
  $: totalInternalCost = lineItems.reduce((sum, item) => sum + ((item.internalCost || 0) * item.quantity), 0);
...
<div class="max-h-[85vh] overflow-y-auto pr-2">
  <!-- Quote Data Import Button - show at the top of the form when originating quote is present -->
  {#if job.originatingQuoteId && quoteLineItems.length > 0 && lineItems.length === 0}
    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
      <div class="flex flex-col items-center">
        <h3 class="text-lg font-medium text-blue-800">Import Quote Data</h3>
        <p class="text-blue-600 mb-4 text-center">
          This job was created from Quote #{quote?.quoteNumber}. You can import all pricing data from the quote to populate this form.
        </p>
        <button
          type="button"
          on:click={addAllQuoteItems}
          class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded shadow-sm transition-colors flex items-center"
        >
                ...
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    {#each lineItems as item (item.id)}
                      {@const margin = ((item.unitPrice - (item.internalCost || 0)) / item.unitPrice * 100) || 0}
                      <tr>
