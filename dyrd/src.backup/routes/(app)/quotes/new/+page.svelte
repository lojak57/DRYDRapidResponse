          ...
          {#if showLineItemWizard}
            <div class="mb-6 border border-blue-200 rounded-lg bg-blue-50 p-4">
              ...
              <div class="grid gap-4 mb-4">
                ...
                {:else if lineItemWizard.category === 'Equipment'}
                  ...
                  <div class="border-t border-blue-200 pt-4">
                    <div class="mb-4">
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {#each [
                          { title: 'Dehumidifier', price: 75, description: 'Commercial grade dehumidifier' },
                          { title: 'Air Mover', price: 45, description: 'High-velocity air mover' },
                          { title: 'Air Scrubber', price: 95, description: 'HEPA air filtration system' },
                          { title: 'Negative Air Machine', price: 125, description: 'For containment areas' },
                          { title: 'Thermal Imaging Camera', price: 150, description: 'For moisture detection' },
                          { title: 'Moisture Meter', price: 35, description: 'For daily readings' },
                          { title: 'Infrared Heater', price: 65, description: 'For targeted drying' },
                          { title: 'Water Extraction Unit', price: 185, description: 'For water removal' }
                        ] as equipment}
                          <button 
                            type="button"
                            class="p-2 border rounded text-left text-sm transition-all {lineItemWizard.description === `${equipment.title} Rental - ${equipment.description}` ? 'bg-blue-100 border-blue-400 shadow-sm' : 'border-gray-300 hover:bg-gray-50'}"
                            on:click={() => {
                              lineItemWizard.description = `${equipment.title} Rental - ${equipment.description}`;
                              lineItemWizard.unitPrice = equipment.price;
                              lineItemWizard.internalCost = Math.round(equipment.price * 0.6 * 100) / 100;
                            }}
                          >
                            <div class="font-medium">{equipment.title}</div>
                            <div class="flex justify-between text-xs">
                              <span class="text-gray-600">{equipment.description}</span>
                              <span class="font-medium text-blue-700">${equipment.price}/day</span>
                            </div>
                          </button>
                        {/each}
                      </div>
                ...
                {:else}
                  <!-- Description for non-specialized items -->
                  <div>
