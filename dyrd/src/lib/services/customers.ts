...
import { customers as customersStore } from '$lib/stores/customerStore';
...
/**
 * Load all customers into the store
 */
export async function loadCustomers(): Promise<void> {
  console.log('Loading customers...');
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 300));
  customersStore.set([...mockCustomers]);
  console.log(`Loaded ${mockCustomers.length} customers.`);
  return;
}
