...
export interface WorkflowTask {
    ...
    dependsOn?: string[];
    // Optional: Condition to determine if task should be shown
    condition?: (job: Job) => boolean;
    // Optional: Action to perform after task completion
    completeAction?: (job: Job) => Partial<Job>;
}

export const TASKS_BY_STATUS: { [key in JobStatus]?: WorkflowTask[] } = {
    [JobStatus.NEW]: [
        { 
            id: 'schedule_job_consolidated', 
            label: 'Schedule Job', 
            requiredRole: [Role.OFFICE, Role.ADMIN],
            description: 'Schedule the job date, assign a technician and truck',
            condition: (job) => !job.scheduledStartDate || !job.assignedUserIds || job.assignedUserIds.length === 0,
            completeAction: (job) => {
                console.log('Running completeAction for schedule_job_consolidated - setting job status to SCHEDULED');
                return {
                    ...job,
    ...
    [JobStatus.SCHEDULED]: [
        { id: 'confirm_dispatch', label: 'Confirm Technician Dispatched', requiredRole: [Role.OFFICE, Role.ADMIN, Role.TECH] }
    ],
    ...
    [JobStatus.INVOICED]: [
        { id: 'record_payment', label: 'Record Payment', requiredRole: [Role.OFFICE, Role.ADMIN], description: 'Apply payment to the invoice and mark the job as paid' }
    ]
}; 