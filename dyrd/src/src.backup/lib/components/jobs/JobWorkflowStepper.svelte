<script lang="ts">
  ...
  function areAllTasksCompletedForStatus(status: JobStatus): boolean {
    ...
    if (status === JobStatus.NEW) {
      // For NEW status, need schedule date and assigned technicians and status should be SCHEDULED
      // Check first if job status has already been advanced to SCHEDULED
      if (job.status === JobStatus.SCHEDULED || job.status === JobStatus.IN_PROGRESS || 
          job.status === JobStatus.PENDING_COMPLETION || job.status === JobStatus.COMPLETED) {
        console.log('Job already advanced past NEW status');
        return true;
      }
      
      // Check if job has been scheduled properly
      ...
      // or job status should already be past SCHEDULED
      if (job.status === JobStatus.IN_PROGRESS || job.status === JobStatus.PENDING_COMPLETION || 
          job.status === JobStatus.COMPLETED) {
        console.log('Job already advanced past SCHEDULED status');
        return true;
      }
