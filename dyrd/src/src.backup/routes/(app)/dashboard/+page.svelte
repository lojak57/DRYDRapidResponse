...
<script lang="ts">
  ...
  // Dropdown state
  let pendingCompletionExpanded = false;
  let paidJobsExpanded = false;
          ...
          {#each sortedCategories as category, i}
            <div class="card-enter {animated ? 'animated' : ''}" style="transition-delay: {i * 75}ms">
              <div class="category-card card-shadow"
                   on:click={() => {
                     if (category.title === "Ready to Start") {
                       goto('/jobs?needsStart=true');
                     } else {
                       goto(`/jobs?status=${encodeURIComponent(category.status)}`);
                     }
                   }}
                   on:keypress={(e) => {
                     if (e.key === 'Enter') {
      ...
      <div class="mb-6">
        <div class="card-glass rounded-lg card-shadow overflow-hidden">
          <div class="p-4 card-header-burgundy">
            <div class="flex justify-between items-center cursor-pointer" on:click={() => pendingCompletionExpanded = !pendingCompletionExpanded}>
              <h2 class="font-bold text-xl flex items-center text-pink-800">
                ...
                <span class="ml-2 bg-pink-200 text-pink-800 px-2 py-0.5 rounded-full text-sm">
                  {pendingCompletionJobs.length}
                </span>
              </h2>
              <button class="text-pink-800 p-1 transition-transform duration-200" class:rotate-180={pendingCompletionExpanded} aria-label="Toggle section">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="mb-6">
        <div class="card-glass rounded-lg card-shadow overflow-hidden">
          <div class="p-4 card-header-emerald">
            <div class="flex justify-between items-center cursor-pointer" on:click={() => paidJobsExpanded = !paidJobsExpanded}>
              <h2 class="font-bold text-xl flex items-center text-emerald-800">
                ...
                Paid & Closed Jobs
                <span class="ml-2 bg-emerald-200 text-emerald-800 px-2 py-0.5 rounded-full text-sm">
                  {paidJobs.length}
                </span>
              </h2>
              <button class="text-emerald-800 p-1 transition-transform duration-200" class:rotate-180={paidJobsExpanded} aria-label="Toggle section">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              </button>
